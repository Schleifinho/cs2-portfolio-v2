...
import {AlertTriangle, ShieldCheck, Mail, Camera, Lock} from "lucide-react";
...
export const Profile = () => {
  ...
  const getPriorityRole = () => {
    ...
    if (user.roles.includes("Mod")) return {
      label: "Moderator",
      color: "text-blue-500 bg-blue-500/10 border-blue-500/20"
    };
    if (user.roles.includes("NoEmailVerification")) return {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 w-full max-w-3xl mx-auto space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-1 shrink-0 px-1">
        <h2
          className="text-xl sm:text-2xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
          Account Settings
        </h2>
            ...
            <div className="flex flex-col space-y-4 min-w-0">
              <div className="space-y-1 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-2 text-muted-foreground">
                  ...
                  {user.confirmedEmail ? (
                    <Badge
                      className="w-fit mx-auto md:mx-0 bg-green-500/10 text-green-600 border-green-500/20 text-[10px] uppercase font-black">
                      Verified
                    </Badge>
                  ) : (
                    <Badge variant="outline"
                           className="w-fit mx-auto md:mx-0 bg-red-500/10 text-red-600 border-red-500/20 text-[10px] uppercase font-black animate-pulse">
                      Unverified
                    </Badge>
                  )}
                </div>
              ...
              {!user.confirmedEmail && (
                <div
                  className="flex flex-col sm:flex-row items-center gap-3 p-3 rounded-lg border border-amber-500/20 bg-amber-500/5 text-amber-700">
